/**
 * Chico Thai Cuisine — Accessibility Tools Widget
 * Same tools as Delhi to Kathmandu. Vanilla JS · WCAG 2.1/2.2 AA · resets on page refresh
 */
(function () {
  'use strict';
  ...
  function applySettings() {
    ...
    /* This site uses px font sizes, so html { font-size } does not
       resize copy. Zoom the page body instead; the widget lives on
       <html> and stays a usable size. */
    ...
    /* Apply visual filters to body so widget UI stays crisp.
       Skip the identity filter — any filter on body makes position:fixed
       descendants (hamburger drawer, lightbox) contain/clip incorrectly. */
    var filter = buildFilter();
    var identity = !state.grayscale && state.saturation === 100 && state.brightness === 100;
    document.body.style.filter = identity ? '' : filter;
